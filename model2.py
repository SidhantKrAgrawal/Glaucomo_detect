...
def load_and_preprocess_image(image_path, image_size=(120, 200)):
    ...
    data = pd.DataFrame({'File Path': [image_path], 'Final Label': ['NRG']})
...
def predict_labels(model_path, image_path):
    ...
    # Output the predicted labels of the image belonging to the positive class using prediction
    labels = [prediction[0][i] > 0.5 for i in range(len(prediction[0]))]
    
    return labels


# Example usage
# model2_path = 'task2.h5'
# image_path = 'TRAIN000034.JPG'

# likelihood2 = predict_labels(model2_path, image_path)
# print(f"The likelihood of the image being positive is: {likelihood2}")